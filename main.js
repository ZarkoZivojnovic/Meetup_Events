...
function showEventList(event) {
    let lon, lat;
    for (let option of document.getElementById("listOfCities").childNodes) {
    ...
    let waitingForCityEvents = new Promise((resolved) => {
        fetch(createEventUrl(lon, lat), function (res) {
            resolved(res);
        });
    });
    waitingForCityEvents.then(res => {
        allEvents = res.data.events;
        document.getElementById("events").innerHTML = renderEventsList(allEvents);
        document.getElementById("events").addEventListener("click", clickOnEvent)
    });
}
...
function eventModal(showOrHide) {
    ...
    if (showOrHide === "show") {
        ...
        setTimeout(() => {
            ...
            modal.style.top = document.documentElement.scrollTop + 100 + "px";
